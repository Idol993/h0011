...
const COLORS = ['#ea580c', '#f97316', '#fb923c', '#fdba74', '#fed7aa', '#ffedd5', '#c2410c'];
...
export default function App() {
  ...
  const refreshAll = useCallback(async () => {
    try {
      const [pRes, tRes, rRes] = await Promise.all([
        ...
        fetch(`/api/reports/weekly${weekOffset ? `?date=${getWeekDate(weekOffset)}` : ''}`)
      ]);
      const [p, t, r] = await Promise.all([pRes.json(), tRes.json(), rRes.json()]);
      setProjects(p);
      setActiveTimer(t);
      setWeeklyReport(r);
    } catch (e) {
  ...
  function exportCSV() {
    const url = `/api/reports/weekly/csv${weekOffset ? `?date=${getWeekDate(weekOffset)}` : ''}`;
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '420px 1fr', gap: 20 }}>
          ...
          <div style={{
            ...
            {weeklyReport && weeklyReport.projects.length === 0 ? (
              <div style={{
                textAlign: 'center',
                padding: '60px 20px',
                color: '#9ca3af',
